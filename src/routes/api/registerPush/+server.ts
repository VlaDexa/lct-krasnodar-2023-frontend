...
async function insertUser(user: PushSubscription) {
	const id_rows = await pool.sql`INSERT INTO requests (endpoint, expiration_time, p256dh_key, auth_key) VALUES (${user.endpoint},${user.expirationTime},${user.keys.p256dh},${user.keys.auth}) RETURNING id`;
